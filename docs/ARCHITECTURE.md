...
│       ├── coverage.ts     What the analyzer could and could not see
...
| `HealthScore`      | score, label (incl. `unrated`), `confidence`, breakdown of `{ signal, delta, note }`                                                                                                                                                                                            |
...
| `AnalysisCoverage` | source files, how many are connected by a resolved import, `level` (`full`/`partial`/`minimal`), languages that cannot be fully resolved, and a note for the user                                                                                                               |
...
7. **Coverage** (`coverage.ts`). What share of source files ended up connected to another file by a resolved import, which significant languages cannot be fully resolved, and a level: `full` (≥40%), `partial`, `minimal` (<10%). This is the input that lets later stages distinguish "nothing wrong" from "nothing visible".
8. **Score** (`score.ts`). Start at 100, apply deltas per signal, clamp, label. The breakdown is part of the result so the UI can show it. Two rules govern the deltas. Warnings the detector reported at `info` severity never score — that is what `info` means, and scoring them anyway penalised projects for a benchmarks directory having no tests of its own. And count-based penalties are expressed as rates through the `rate()` helper: `n / denominator / full` of the maximum, where `full` is the share at which a signal costs everything. Absolute counts made the score a proxy for repository size (`r = -0.73` against file count across the corpus; `npm run bench` reports that correlation on every run).
   At `minimal` coverage the graph-derived signals — cycles, module cohesion, dead modules — are withheld with a note saying so rather than scored as clean, `confidence` is `limited`, and the label becomes `unrated`: withholding penalties _raises_ the number, so a repository the analyzer cannot see would otherwise outscore one it can.
9. **Summary** (`summary.ts`). `buildTemplateSummary` writes the prose from facts; `SummaryProvider.summarize(draft, facts)` may rewrite `headline`/`description`/`architecture`. `providerFromEnv` returns the template provider unless `REPOSCOPE_LLM_URL` and `REPOSCOPE_LLM_MODEL` are set.
